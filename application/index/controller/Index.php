...
class Index extends Controller
{
    ...
    protected function initialize()
    {
        ...
        //软件版本
        $this->view->rjbnode = WebSite::where('if_rjnode', '>', 0)->select();
    }
    ...
    public function parse($link = '', $debug = '', $verify = '')
    {
        ...
        if ($result['code'] === 'verify') {
            return json(['code' => -1, 'msg' => $result['msg'], 'data' => $result['verify']]);
        }
        //77网盘的滑轮验证
        if ($result['code'] === 'qqhlyz') {
           // return $this->error($result['verify']);
            return json(['code' => -2, 'msg' => $result['msg'], 'data' => $result['verify']]);
        }
        if ($result['code'] !== 1) {
