...
class ParseUrl
{
    ...
    public function get_nipic_com()
    {
        ...
        if (!empty($token['1'])) {
            ...
            $http_curl = new HttpCurl('http://down.nipic.com/ajax/download_go', 'POST',
                'id=' . $site_code['1'] . '&kid=2&__RequestVerificationToken=' . $token['1'],
            ...
            if (!empty($result['data']['url'])) {
                ...
                $http_curl       = new HttpCurl(str_replace(' ', '%20', $result['data']['url']), 'GET', null, $header3, true, $this->cookie['cookie_id']);
                $response_header = $http_curl->request_curlopts([
                    CURLOPT_FOLLOWLOCATION => false,
                    CURLOPT_MAXREDIRS      => 0,
                ])->send_request()->get_response_header();
    ...
    public function get_88tph_com()
    {
        ...
        $http_curl = new HttpCurl('https://www.88tph.com/geturl', 'POST', 'doc=' . $site_code['1'] . '&vip=true', $header, true, $this->cookie['cookie_id']);
        $result    = $http_curl->send_request()->get_response_body();
        $result    = json_decode($result, 1);
    ...
    public function get_125pic_com()
    {
        ...
        $http_curl = new HttpCurl('http://www.125pic.com/api/sucai/download', 'POST', '{"id":' . $site_code['1'] . ',"t":' . Request::time() . random(3, 1) . '}', $header, true, $this->cookie['cookie_id']);
        $result    = $http_curl->send_request()->get_response_body();
        $result    = json_decode($result, 1);
        if (!empty($result['data']['url'])) {
    ...
    public function get_photophoto_cn()
    {
        ...
        foreach ($matchs[1] as $key => $url) {
            ...
            $host            = explode('photophoto.cn/', $host);
            $header2['Host'] = $host[0] . 'photophoto.cn';
            ...
            if (!empty($response_header['location'])) {
                $download[$matchs[2][$key]] = $response_header['location'];
            } else if (!empty($response_header['redirect_url'])) {
                $download[$matchs[2][$key]] = $response_header['redirect_url'];
            }
    ...
    public function get_669pic_com()
    {
        ...
        $http_curl = new HttpCurl('http://669pic.com/?c=Ajax&a=ajaxDownload', 'POST', 'id=' . $site_code['1'] . '&pictype=1&s_id=', $header, true, $this->cookie['cookie_id']);
        $result    = $http_curl->send_request()->get_response_body();
        $result    = json_decode($result, 1);
        if (!empty($result['link'])) {
            return ['code' => 1, 'site_code_type' => '', 'site_code' => $site_code['1'], 'msg' => ['立即下载' => $result['link']]];
