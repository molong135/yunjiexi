<?php /*a:2:{s:73:"F:\php\81sc\update\6-15\qtsc.shzmzxw\application\admin\view\user_edit.php";i:1556300794;s:70:"F:\php\81sc\update\6-15\qtsc.shzmzxw\application\admin\view\layout.php";i:1556520742;}*/ ?>
<!DOCTYPE html>
...
<div class="admin-content">
	<div class="p-3"><link rel="stylesheet" type="text/css" href="static/css/jquery.datetimepicker.css">
<form class="card" method="post" autocomplete="off" action="<?php echo url('admin/user/edit'); ?>">
    <input type="hidden" name="uid" value="<?php echo htmlentities($user['uid']); ?>">
    <div class="card-header">创建用户</div>
    <div class="card-body">
        <div class="form-group">
            <label>用户名</label>
            <input type="text" name="username" class="form-control" value="<?php echo htmlentities($user['username']); ?>">
        </div>
        <div class="form-group">
            <label>手机号</label>
        ...
        <div class="form-group">
            <label>邮 箱</label>
            <input type="text" name="email" class="form-control" value="<?php echo htmlentities($user['email']); ?>">
        </div>
        <div class="form-group">
            <label>密码</label>
        ...
        <div class="form-group">
            <label>最大解析次数</label>
            <input type="number" name="parse_max_times" class="form-control" value="<?php echo htmlentities($user['out_time']); ?>">
        ...
        <?php if($user['out_time'] > 0 && $user['out_time'] < 315360000): ?>
            <div class="form-group">
                <label>过期时间</label>
                <input type="number" name="out_time" class="form-control" value="<?php echo htmlentities($user['out_time']/3600); ?>">
        ...
        <?php elseif($user['out_time'] > 315360000): ?>
            <div class="form-group">
                <label>过期时间</label>
                <input type="text" name="out_time" class="form-control" data-toggle="date-picker" value="<?php echo date('Y-m-d H:i',$user['out_time']); ?>">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="type">
                <option value="member" <?php if($user['type'] == 'member'): ?>selected<?php endif; ?>>会员</option>
                <option value="proxy" <?php if($user['type'] == 'proxy'): ?>selected<?php endif; ?>>代理</option>
                <option value="system" <?php if($user['type'] == 'system'): ?>selected<?php endif; ?>>管理员</option>
            </select>
        ...
        <?php foreach($site_list as $site): ?>
            <input type="hidden" name="site_access[<?php echo htmlentities($site['site_id']); ?>][day_used]" value="<?php echo isset($user['site_access'][$site['site_id']]['day_used']) ? htmlentities($user['site_access'][$site['site_id']]['day_used']) : '0'; ?>">
            <input type="hidden" name="site_access[<?php echo htmlentities($site['site_id']); ?>][max_used]" value="<?php echo isset($user['site_access'][$site['site_id']]['max_used']) ? htmlentities($user['site_access'][$site['site_id']]['max_used']) : '0'; ?>">
            <div class="form-group d-flex justify-content-between">
                ...
                <div class="input-group mr-3">
                    ...
                    <input type="number" class="form-control" name="site_access[<?php echo htmlentities($site['site_id']); ?>][day]" placeholder="日解析最大次数" value="<?php echo isset($user['site_access'][$site['site_id']]['day']) ? htmlentities($user['site_access'][$site['site_id']]['day']) : '0'; ?>">
                </div>
                <div class="input-group">
                    <div class="input-group-prepend">
                        <div class="input-group-text">最大解析总次数</div>
                    </div>
                    <input type="number" class="form-control" name="site_access[<?php echo htmlentities($site['site_id']); ?>][all]" placeholder="最大解析总次数" value="<?php echo isset($user['site_access'][$site['site_id']]['all']) ? htmlentities($user['site_access'][$site['site_id']]['all']) : '0'; ?>">
                </div>
            </div>
        <?php endforeach; ?>
    </div>
